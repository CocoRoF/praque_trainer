...
def qwen3_formatter(examples, tokenizer):
...
def qwen3_sft_columns_to_messages(examples):
    system = examples['system']
    question = examples['question']
    answer = examples['answer']
    if system is None or len(system) < 1:
...
def processor(
    ...
    if selected_trainer_name == 'none':
        ...
        try:
            if train_dataset.column_names == ['messages']:
                ...
                new_train_dataset = train_dataset.map(lambda examples: qwen3_messages_to_clm_formatter(examples, tokenizer))
                new_train_dataset = new_train_dataset.select_columns(['text'])
                train_dataset = new_train_dataset

                if test_dataset:
                    test_dataset = messages_form_dataset_filter(test_dataset)
                    test_dataset = test_dataset.map(lambda examples: qwen3_messages_to_clm_formatter(examples, tokenizer))
                ...
                print("[INFO] qwen3 Dataset Columns Renamed. ['text']")
                ...
                print("[INFO] qwen3 Dataset Columns Renamed. ['system', 'question', 'answer']")

        except:
            raise ValueError("qwen3 Dataset Columns Error")
    ...
    if selected_trainer_name == 'SFT Trainer' or selected_trainer_name == "KL-SFT Trainer":
        ...
        try:
            if train_dataset.column_names == ['messages']:
                train_dataset = messages_form_dataset_filter(train_dataset)
                print("[INFO] qwen3 Dataset Columns Already Renamed. ['messages']")
            else:
                train_dataset = rename_dataset_columns(train_dataset, 'system', 'question', 'answer')
                if test_dataset:
                    test_dataset = rename_dataset_columns(test_dataset, 'system', 'question', 'answer')
        ...
        try:
            if train_dataset.column_names == ['messages']:
                ...
                train_dataset = train_dataset.map(lambda examples: qwen3_sft_formatter(examples))
                try:
                    test_dataset = test_dataset.map(lambda examples: qwen3_sft_formatter(examples))
                except:
                    test_dataset = None
                print("[INFO] qwen3 Apply Chat Template. Column Name: text")
        except:
            raise ValueError("[FATAL ERROR] qwen formatter Fail.")
        ...
        if len(train_dataset.column_names) == 3:
            try:
                ...
                print("[INFO] qwen3 Dataset Columns Renamed. ['prompt', 'chosen', 'rejected']")
            except:
                raise ValueError("qwen3 Dataset Columns Error")
            ...
            try:
                ...
                print("[INFO] qwen3 Dataset Columns Renamed. ['system', 'prompt', 'chosen', 'rejected']")
            except:
                raise ValueError("qwen3 Dataset Columns Error")
