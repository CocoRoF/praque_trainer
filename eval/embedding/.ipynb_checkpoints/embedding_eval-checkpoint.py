...
# 기존 컬럼 positive_field와 negative_field의 첫 번째 원소의 'text' 값을 추출하여 덮어씁니다.
    example[positive_field] = example[positive_field][0]['text']
    example[negative_field] = example[negative_field][0]['text']
    return example
...
def evaluate_retriever_np_ndvg(model, tokenizer, datasets, query_field, positive_field, negative_field, device='cuda', batch_size=32, gpu_num=1):
    ...
    from torch.utils.data import DataLoader
    from tqdm.auto import tqdm  # tqdm import, 만약 없으면 추가
    ...
    datasets = datasets.map(extract_text, positive_field, negative_field)
    dataloader = DataLoader(datasets, batch_size=batch_size, shuffle=False)

    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Evaluating Retriever (batched)"):
            # 배치 내 각 sample의 type과 일부 내용을 로깅
            for idx, sample in enumerate(batch):
                logging.debug("Batch sample %d type: %s, content: %s", idx, type(sample))
            
            # 각 샘플에서 query, positive, negative 필드를 추출 (예외 발생 시 로깅 후 중단)
            try:
                queries = [sample[query_field] for sample in batch]
                positives = [sample[positive_field] for sample in batch]
                negatives = [sample[negative_field] for sample in batch]
            except Exception as e:
                logging.error("에러 발생: 배치 내 필드 접근 중 오류 발생. Batch: %s, error: %s", batch, str(e))
                raise e
...
def evaluate_sts(model, tokenizer, dataset, batch_size=32, s1='sentence1', s2='sentence2', label='labels', device='cuda'):
    ...
    for batch in tqdm(dataloader, desc="Evaluating STS"):
        # 문장과 라벨 추출
        sentences1 = batch[s1]
        sentences2 = batch[s2]
        labels = batch[label]
...
def evaluate_retriever_ndcg(model, tokenizer, all_datasets, s1, s2, s3, label, top_k =5, device='cuda', batch_size=32):
    ...
    # text 컬럼명을 query_text, corpus_text로 변경 (필요에 따라 corpus_text1 등으로 수정 가능)
    queries_df = queries_df.rename(columns={'text': 'query_text'})
    corpus_df = corpus_df.rename(columns={'text': 'corpus_text'})
    
    # default 데이터셋과 queries, corpus 데이터셋을 각각 query-id, corpus-id 기준으로 병합
    ...
    # 최종적으로 query_text, corpus_text, score 컬럼만 선택
    ...
    # 각 샘플에 대해 결과를 저장할 리스트 (각 항목은 {'query_text': ..., 'cosine_sim': ..., 'score': ...})
    results = []
    
    for batch in tqdm(dataloader, desc="Evaluating STS with NDCG"):
        # 배치에서 query와 corpus 텍스트, 그리고 라벨(score) 추출
        ...
        # 임베딩 추출 (pooler_output가 없으면 CLS 토큰 사용)
        ...
        # 각 배치의 query와 corpus 임베딩에 대해 cosine similarity 계산 (배치 내 각 샘플별)
    ...
    # query_text를 기준으로 그룹핑하여 각 query에 대해 ranking 구성
    ndcg_scores = []
    query_groups = {}
    ...
    # 각 query 그룹별로 NDCG 계산
    for query, items in query_groups.items():
        # cosine similarity 내림차순 정렬 후 상위 top_k 결과만 사용
        ...
        # 이상적인 ranking: ground truth score 내림차순 정렬 후 상위 top_k 결과만 사용
